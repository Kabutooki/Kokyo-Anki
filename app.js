/* 公共一問一答 v24 - explicit study sessions + configurable question count */
'use strict';
...
const UPDATE_READY_KEY='kokyo_flashcards_update_ready_v24';
...
function clampSessionSize(v){return clampInt(v||DEFAULT_SESSION_SIZE,1,Math.max(1,CARDS.length||1925))}
...
function render(){
  ...
  const c=deck[state.index],status=statusOf(c),opts=choicesFor(c);$('#sectionBadge').textContent=`${cardSection(c)} ${cardSectionTitle(c)}`;const ib=$('#impBadge');ib.textContent=impLabel[impOf(c)];ib.className='badge '+impOf(c);$('#statusBadge').textContent=statusLabel[status]||status;$('#sourceInfo').textContent=`一問一答 p.${c.source.page} / #${c.source.number}`;$('#question').textContent=promptOf(c);$('#answer').textContent=displayAnswer(answerOf(c));
